...
public class LearningResourceControllerTests {
    ...
    @Test
    public void testSaveLearningResources() throws Exception{
        List<LearningResource> learningResourceList=new ArrayList<>(Arrays.asList(learningResource1,learningResource2));
        this.mockMvc.perform(MockMvcRequestBuilders.post("/learningresources/v1/").contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(learningResourceList))).andExpect(status().isCreated());
    }

    @Test
    public void testDeleteResource() throws Exception{
        int resourceId=1234;
        this.mockMvc.perform(MockMvcRequestBuilders.delete("/learningresources/v1/learningresource/" + resourceId)).andExpect(status().isOk());
    }
